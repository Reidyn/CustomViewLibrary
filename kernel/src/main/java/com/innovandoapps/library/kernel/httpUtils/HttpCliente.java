...
public class HttpCliente {
    ...
    public JSONObject sendJsonPost(String url,JSONObject jsonObject,String cookie){
        String respuesta = httpConnetar(jsonObject,url,cookie,0);
        try {
    ...
    public String sendDelete(String url,String cookie){
        String respuesta = httpConnetar(null,url,cookie,3);
        return respuesta;
    }

    public JSONObject sendJsonPut(String url,JSONObject jsonObject,String cookie){
        String respuesta = httpConnetar(jsonObject,url,cookie,2);
        try {
            JSONObject object = new JSONObject(respuesta);
            return object;
        } catch (JSONException e) {
    ...
    private String httpConnetar(JSONObject jsonObject,String urlWebserver,String cookie,int metodo){
        String response = "";
        try {
            url = new URL(urlWebserver);
            connection = (HttpURLConnection)url.openConnection();
    ...
    public String httpUpFile(String urlWebserver,File file,String keyfileparameter,String cookie){
        String response = "";
        String lineEnd = "\r\n";
        String twoHyphens = "--";
        String boundary = "*****";
        try {
            ...
            dos.writeBytes(twoHyphens + boundary + lineEnd);
            dos.writeBytes("Content-Disposition: form-data; name=\""+keyfileparameter+"\";filename=\"" + file.getName() +"\"" + lineEnd);
            dos.writeBytes("Content-Length: " + file.length() + lineEnd);
            dos.writeBytes(lineEnd);
